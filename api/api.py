...
class saml:
    def extract_xml_attributes():
        if request.method == 'POST':
            ...
            request_body_in_string = operations.decode_request_body_to_string(request_body)
            xml_body = operations.get_xml_body(request_body_in_string)
    ...
    def get_certificate_with_header():
        if request.method == 'POST':
            ...
            certificate_in_string = operations.decode_request_body_to_string(certificate_data)
            formatted_certificate = operations.format_certificate(certificate_in_string)
    ...
    def format_certificate():
        if request.method == 'POST':
            certificate_data = request.get_data();
            cert_data_in_string = operations.decode_request_body_to_string(certificate_data)
            format_cert = operations.format_certificate(cert_data_in_string)
            return format_cert
        else:
    ...
    def get_xml_content():
        if request.method == 'POST':
            request_body = request.get_data()
            xml_body_in_string = operations.decode_request_body_to_string(request_body)
            xml_body = operations.get_xml_body(xml_body_in_string)
    ...
    def validate_entity_id():
        if request.method == 'POST':
            request_body = request.get_data()
            entityId = operations.decode_request_body_to_string(request_body)
