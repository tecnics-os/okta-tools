...
class TestJwtViewer(unittest.TestCase):
    ...
    def test_JWT_header_typ(self):
        data = jwt_viewer.decode_encoded_string(TestJwtViewer.encoded_token)
        self.assertEqual(data["header"]["typ"],"JWT")
    
    def test_JWT_payload_sub(self):
        data = jwt_viewer.decode_encoded_string(TestJwtViewer.encoded_token)
        self.assertEqual(data["payload"]["sub"],"1234567890")
    
    def test_JWT_payload_name(self):
        data = jwt_viewer.decode_encoded_string(TestJwtViewer.encoded_token)
        self.assertEqual(data["payload"]["name"],"John Doe")